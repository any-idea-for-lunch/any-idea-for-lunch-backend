...
-   프로젝트 내 HTTPS 인증서 구성 없이 HTTP로 배포할 경우  
    브라우저가 위치 권한 요청을 전면 차단
-   로컬 개발 환경에서는 문제 없음

컨테이너 내부 서비스에 인증서를 포함시키거나 SSL Termination 구성을 시도했지만  
현 시점에서 프로젝트 범위상 적용이 어렵다는 판단이 내려졌습니다.
...
-   향후 배포 고려 시 초기 설계 단계에서 SSL 적용 범위와 방식을 사전 확정해야 함
