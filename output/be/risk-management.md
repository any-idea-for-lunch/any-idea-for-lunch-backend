...
-   NAS의 Docker 컨테이너 구동은 정상 동작
-   **HTML5 위치 정보 기능은 HTTPS 환경에서만 동작**
-   NAS 자체는 HTTPS를 지원하지만  
    컨테이너 내부 서비스에도 **별도의 HTTPS 인증서 적용이 필요**
-   인증서 없이 HTTP로 서비스할 경우  
    브라우저가 위치 권한 요청을 차단
-   로컬 개발 환경에서는 문제 없음
...
배포 예정 URL:  
[http://hongnass.synology.me:8888/](http://hongnass.synology.me:8888/)

현재 해당 링크는 접근 가능하도록 서버를 오픈해둔 상태입니다.  
페이지 자체는 정상적으로 표시되지만, **HTTP 환경으로 인해 브라우저가 위치 권한을 거부**합니다.  
따라서 지도 및 위치 기반 데이터가 로딩되지 않는 현상을 직접 확인할 수 있습니다.

### 해결 조치

-   Docker 기반 NAS 배포 시 인증서 구성 없이 위치 기반 기능 사용 불가 확인
-   브라우저 보안 정책 우회 불가
-   **HTTPS 인증서 적용 전까지 로컬 환경 실행으로 방향 전환**

### 결과 및 교훈

-   위치 기반 서비스의 핵심 조건은 HTTPS
-   NAS의 HTTPS 지원 여부와 별개로 **컨테이너 내부 인증서 적용이 필요**
-   Docker 배포 자체는 문제 없으나 **보안 인증 미적용이 핵심 리스크**
-   향후 배포 시 SSL 적용 타이밍과 방식을 초기 설계 단계에서 확정 필요
